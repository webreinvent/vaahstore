...
let model_namespace = 'VaahCms\\Modules\\Store\\Models\\GroupCustomer';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/store/groupcustomers";
...
export const useGroupCustomerStore = defineStore({
    id: 'groupcustomers',
    state: () => ({
        base_url: base_url,
        ajax_url: ajax_url,
    ...
    actions: {
        ...
        async getItemAfter(data, res)
        {
            if(data)
            {
                ...
                this.$router.push({name: 'groupcustomers.index'});
            }
        ...
        toForm()
        {
            this.item = vaah().clone(this.assets.empty_item);
            this.getFormMenu();
            this.$router.push({name: 'groupcustomers.form'})
        },
        //---------------------------------------------------------------------
        toView(item)
        {
            this.item = vaah().clone(item);
            this.$router.push({name: 'groupcustomers.view', params:{id:item.id}})
        },
        //---------------------------------------------------------------------
        toEdit(item)
        {
            this.item = item;
            this.$router.push({name: 'groupcustomers.form', params:{id:item.id}})
        },
        //---------------------------------------------------------------------
        isViewLarge()
        {
...
// Pinia hot reload
if (import.meta.hot) {
    import.meta.hot.accept(acceptHMRUpdate(useGroupCustomerStore, import.meta.hot))
}
