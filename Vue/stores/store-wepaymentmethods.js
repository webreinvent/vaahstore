...
let model_namespace = 'VaahCms\\Modules\\Store\\Models\\WePaymentMehthod';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/store/wepaymentmethods";
...
export const useWePaymentMehthodStore = defineStore({
    id: 'wepaymentmethods',
    state: () => ({
        base_url: base_url,
        ajax_url: ajax_url,
    ...
    actions: {
        ...
        async getItemAfter(data, res)
        {
            if(data)
            {
                ...
                this.$router.push({name: 'wepaymentmethods.index'});
            }
        ...
        toForm()
        {
            this.item = vaah().clone(this.assets.empty_item);
            this.getFormMenu();
            this.$router.push({name: 'wepaymentmethods.form'})
        },
        //---------------------------------------------------------------------
        toView(item)
        {
            this.item = vaah().clone(item);
            this.$router.push({name: 'wepaymentmethods.view', params:{id:item.id}})
        },
        //---------------------------------------------------------------------
        toEdit(item)
        {
            this.item = item;
            this.$router.push({name: 'wepaymentmethods.form', params:{id:item.id}})
        },
        //---------------------------------------------------------------------
        isViewLarge()
        {
...
// Pinia hot reload
if (import.meta.hot) {
    import.meta.hot.accept(acceptHMRUpdate(useWePaymentMehthodStore, import.meta.hot))
}
